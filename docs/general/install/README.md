...
module "cloudfront-origin-request-policy" {
    source = "github.com/pbs/terraform-cloudfront-origin-request-policy-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/cloudfront-origin-request-policy'
gh -R 'pbs/terraform-cloudfront-origin-request-policy-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/cloudfront-origin-request-policy'
rm -f release.tar.gz
```
...
```bash
git remote add -f cloudfront-origin-request-policy git@github.com:pbs/terraform-cloudfront-origin-request-policy-module.git
git subtree add --prefix terraform/modules/cloudfront-origin-request-policy cloudfront-origin-request-policy main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/cloudfront-origin-request-policy cloudfront-origin-request-policy $REF --squash
```
